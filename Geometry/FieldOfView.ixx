...
export namespace fatpound::geometry
{
    ...
    {
        return static_cast<T>(2.0 * std::atan(sensorWidth / 2.0 / focalLength));
    }
    ...
    {
        return static_cast<T>(2.0 * std::atan(Hypotenuse<>(std::tan(hFovRad / 2.0), std::tan(vFovRad / 2.0))));
    }
    ...
    {
        return static_cast<T>(2.0 * std::atan(std::tan(hFovRad / 2.0) * std::sqrt(1.0 + 1.0 / math::Square<>(aspectRatio))));
    }
    ...
    {
        return static_cast<T>(2.0 * std::atan(std::tan(vFovRad / 2.0) * std::sqrt(1.0 + math::Square<>(aspectRatio))));
    }
    ...
    {
        return static_cast<T>(2.0 * std::atan(std::tan(dFovRad / 2.0) / std::sqrt(1.0 + 1.0 / math::Square<>(aspectRatio))));
    }
    ...
    {
        return static_cast<T>(2.0 * std::atan(std::tan(dFovRad / 2.0) / std::sqrt(1.0 + math::Square<>(aspectRatio))));
    }
