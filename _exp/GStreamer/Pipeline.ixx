...
import FatX.GStreamer.IEffectChain;
...
export namespace fatx::gstreamer
{
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //
    // uridecodebin -> audioconvert -> audioresample -> tee +-> queue_identity -> identity ---> input_selector -> audiosink
    //                                                      |                                        ^
    //                                                      |                                        |
    //                                                      |                                        |
    //                                                      +-> queue_wet -> EffectChain ------------+
    //
...
#pragma warning (disable : 4625 4626)
        ...
        struct alignas(64) Data
        {
            ...
            SmartGstPad   dry_tee_pad;
            SmartGstPad   dry_sel_pad;
...
#pragma warning (pop)
    ...
    public:
        explicit Pipeline(std::unique_ptr<IEffectChain> pChain)
            :
            Pipeline()
        {
            if (pChain not_eq nullptr and pChain->Count() > 0)
            {
                DispatchTask_({ .type = Task::Type::AttachEffectChain, .effect = std::move(pChain), .pipeline = this });
            }
        }
...
#pragma warning (pop)
        ...
        ~Pipeline() noexcept
        {
            try
            {
                ...
                DispatchTask_({ .type = Task::Type::DetachEffectChain, .pipeline = this });
    ...
    private:
        ...
        static auto S_TaskHandler_(const gpointer data) noexcept -> gboolean
        {
            ...
            case Task::Type::AttachEffectChain: pipeline.AttachEffectChain_(std::move(task.effect)); break;
            case Task::Type::DetachEffectChain: pipeline.DetachEffectChain_();                       break;
        ...
        static void S_PadAddedHandlerOf_uridecodebin_([[maybe_unused]] GstElement* const src, GstPad* const newPad, Data* const data) noexcept
        {
            ...
            LinkPads(newPad, audioconvert_1_sink_pad);
    ...
    private:
        ...
        void SetUri_              (const std::string& uriPath) noexcept
        {
            ...
            {
                g_printerr("Cannot assign uriPath to m_loaded_uri_!: %s\n", ex.what());
            }
            catch (...)
            {
...
#pragma warning (pop)
        ...
        void SetupIdentityBranch_ () noexcept
        {
            ...
            m_data_.dry_tee_pad = SmartGstPad{ m_data_.tee, gst_element_request_pad_simple(m_data_.tee, "src_%u") };

            if (not m_data_.dry_tee_pad)
            {
                g_printerr(__func__);
                g_printerr(": failed to request tee dry src pad.\n");
...

            {
                ...
                LinkPads(m_data_.dry_tee_pad.Get(), drySinkPad.get());
            }

            m_data_.dry_sel_pad = SmartGstPad{ m_data_.input_selector, gst_element_request_pad_simple(m_data_.input_selector, "sink_%u")};

            if (not m_data_.dry_sel_pad)
            {
                g_printerr(__func__);
                g_printerr(": failed to request input-selector dry sink pad.\n");

                return;
            }

            g_print("Obtained 'identity_pad': %s\n", gst_pad_get_name(m_data_.dry_sel_pad.Get()));

            {
                const auto drySrcPad = UniqueGstPtr<GstPad>{gst_element_get_static_pad(m_data_.identity, "src") };

                LinkPads(drySrcPad.get(), m_data_.dry_sel_pad.Get());
            }
        }
        ...
        void AttachEffectChain_   (std::unique_ptr<IEffectChain> pChain) noexcept
        {
            ...
            if ((pChain == nullptr) or (pChain->Count() == 0))
            {
                g_printerr(__func__);
                g_printerr(": null or empty chain, ignoring.\n");

                return;
            }

            if (m_pEffectChain_ not_eq nullptr)
            {
                g_printerr(__func__);
                g_printerr(": a chain is already attached. Detach first.\n");
...
#pragma warning (disable : 4625 4626)
            ...
            struct RollbackGuard final
            {
                ...
                GstElement*   chain_bin{};
                SmartGstPad   tee_effect_pad;
                SmartGstPad   sel_effect_pad;
                bool          committed{};
                ...
                ~RollbackGuard() noexcept
                {
                    ...
                    g_printerr(__func__);
                    g_printerr(": Rolling back...");

                    sel_effect_pad.Reset();
                    tee_effect_pad.Reset();

                    if (queue_wet not_eq nullptr)
                    {
                        gst_element_set_state(queue_wet, GST_STATE_NULL);
                        gst_bin_remove(GST_BIN(pipeline), queue_wet);
                    }

                    if (chain_bin not_eq nullptr)
                    {
                        gst_element_set_state(chain_bin, GST_STATE_NULL);
                        gst_bin_remove(GST_BIN(pipeline), chain_bin);
                    }
                }
            };
#ifdef FATLIB_BUILDING_WITH_MSVC
#pragma warning (pop)
            ...
            if (gst_bin_add(GST_BIN(m_pPipeline_), pChainBin) == FALSE)
            {
                g_printerr(__func__);
            ...
            rg.chain_bin = pChainBin;
            m_queue_wet_ = CreatePlugin("queue", "queue-wet-1");

            if (m_queue_wet_ == nullptr)
            {
                g_printerr(__func__);
            ...
            if (gst_bin_add(GST_BIN(m_pPipeline_), m_queue_wet_) == FALSE)
            {
                g_printerr(__func__);
                g_printerr(": could not add queue-wet to pipeline.\n");

                gst_object_unref(m_queue_wet_);
                m_queue_wet_ = nullptr;

                return;
            }

            rg.queue_wet = m_queue_wet_;

            gst_element_sync_state_with_parent(m_queue_wet_);
            gst_element_sync_state_with_parent(pChainBin);

            rg.tee_effect_pad = SmartGstPad{ m_data_.tee, gst_element_request_pad_simple(m_data_.tee, "src_%u") };

            if (not rg.tee_effect_pad)
            {
                g_printerr(__func__);
                g_printerr(": failed to request tee src pad.\n");

                return;
            }

            {
                const auto queueSink = UniqueGstPtr<GstPad>{ gst_element_get_static_pad(m_queue_wet_, "sink") };

                LinkPads(rg.tee_effect_pad.Get(), queueSink.get());
            }

            {
                const auto queueSrc  = UniqueGstPtr<GstPad>{ gst_element_get_static_pad(m_queue_wet_, "src") };
                const auto chainSink = UniqueGstPtr<GstPad>{ pChain->GetSinkPad() };

                LinkPads(queueSrc.get(), chainSink.get());
            }

            rg.sel_effect_pad = SmartGstPad{ m_data_.input_selector, gst_element_request_pad_simple(m_data_.input_selector, "sink_%u") };

            if (not rg.sel_effect_pad)
            {
                g_printerr(__func__);
            ...
            {
                const auto chainSrc = UniqueGstPtr<GstPad>{ pChain->GetSrcPad() };

                LinkPads(chainSrc.get(), rg.sel_effect_pad.Get());
            }

            m_tee_effect_pad_ = std::move(rg.tee_effect_pad);
            m_sel_effect_pad_ = std::move(rg.sel_effect_pad);
            rg.committed      = true;
            m_pEffectChain_   = std::move(pChain);

            g_object_set(m_data_.input_selector, "active-pad", m_sel_effect_pad_.Get(), nullptr);
        ...
        void DetachEffectChain_   () noexcept
        {
            ...
            if (m_pEffectChain_ == nullptr)
            {
                g_printerr(__func__);
                g_printerr(": no chain is attached.\n");
            ...
            if (m_data_.dry_sel_pad)
            {
                g_object_set(m_data_.input_selector, "active-pad", m_data_.dry_sel_pad.Get(), nullptr);
            }

            m_sel_effect_pad_.Reset();
            m_tee_effect_pad_.Reset();

            if (m_queue_wet_ not_eq nullptr)
            {
                gst_bin_remove(GST_BIN(m_pPipeline_), m_queue_wet_);
                m_queue_wet_ = nullptr;
            }

            if (auto* const pChainBin = m_pEffectChain_->GetBin(); pChainBin not_eq nullptr)
            {
                gst_bin_remove(GST_BIN(m_pPipeline_), pChainBin);
            }

            m_pEffectChain_.reset();
        }
        ...
        void OnGstStateChanged_   (const bool isPlaying) noexcept
        {
        ...
        void Cleanup_             () noexcept
        {
            ...
            if (GetState_() not_eq GST_STATE_NULL)
            {
                SetState_(GST_STATE_NULL);
            }
    ...
    private:
        ...
        std::unique_ptr<IEffectChain>      m_pEffectChain_;
        GstElement*                        m_queue_wet_{};
        SmartGstPad                        m_tee_effect_pad_;
        SmartGstPad                        m_sel_effect_pad_;
